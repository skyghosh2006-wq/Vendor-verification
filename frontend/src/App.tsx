...
export const App: React.FC = () => {
    ...
    try {
      ...
      if (typeof provider.connect === 'function') {
        try {
          ...
          // Retry without network param if wallet expects no-arg connect
          try {
  ...
  const handleSubmitVerification = async (data: { score: number; taxId: string; secretSalt: string }) => {
    ...
    try {
      ...
      showToast('error', 'Verification Failed', err?.message || 'Transaction submission or ZK proof generation failed.');
    } finally {
