...
import { ShieldCheck, CheckCircle2, AlertCircle, Info, Sparkles, Server, Lock } from 'lucide-react';

export const App: React.FC = () => {
  // Environment variables with fallback configurations
  const network = import.meta.env.VITE_NETWORK || 'undeployed (local devnet)';
  const envContractAddress = import.meta.env.VITE_CONTRACT_ADDRESS || '0x4a91b2c3d4e5f67890abcdef1234567890abcdef';
  ...
  // Auto connect simulation / Lace Wallet listener
  const handleConnectWallet = () => {
    // Check if Lace Midnight wallet extension is present on window
    const windowLace = (window as any).midnight?.lace;
    if (windowLace) {
      windowLace.enable().then((api: any) => {
        setWalletConnected(true);
        setWalletAddress(api.address || 'mn_addr_preprod1q9v8u0x7a2b3c4d5e6f7a8b9c0d1e2f3');
        showToast('success', 'Wallet Connected', 'Connected to Lace Midnight Wallet');
      }).catch((err: any) => {
        showToast('error', 'Connection Error', err?.message || 'Lace connection rejected');
      });
    } else {
      // Standalone devnet demo wallet connection
      setWalletConnected(true);
      setWalletAddress('mn_addr_preprod1q9v8u0x7a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9');
      showToast('success', 'Wallet Connected', 'Connected with Midnight devnet wallet credentials.');
    }
  };
  ...
  const handleDisconnectWallet = () => {
    ...
    showToast('info', 'Disconnected', 'Wallet session terminated.');
  };
  ...
  // Submit private vendor verification ZK circuit
  const handleSubmitVerification = async (data: { score: number; taxId: string; secretSalt: string }) => {
    setIsSubmitting(true);
    showToast('info', 'Generating ZK Proof', 'Executing client-side witness calculations via Compact runtime...');

    try {
      // Simulate client proof computation & tx submission (2.5s)
      await new Promise((resolve) => setTimeout(resolve, 2500));

      // Simple deterministic hash simulation for UI preview
      const encoder = new TextEncoder();
      const bytes = encoder.encode(`${data.secretSalt}:${data.taxId}`);
      let hash = 0n;
      ...
      setLastVerificationTimestamp(Math.floor(Date.now() / 1000));
      setLastVerifiedCommitment(hexHash.slice(0, 66));

      showToast('success', 'Verification Registered!', 'ZK proof verified successfully. Vendor added to public ledger.');
    } catch (err: any) {
  ...
  const handleUpdateMinimumScore = async (newScore: number) => {
    ...
    try {
      ...
      showToast('success', 'Threshold Updated', `Minimum compliance score requirement set to ${newScore}`);
    } catch (err: any) {
